...
@section('content')
    ...
    <div class='panel panel-default scorefilterpanel'>
        <div class='panel-heading'>MD Assessment Scores</div>
        <div class='panel-body'>
                ...
                <div class="row score-filter form-inline">
                    <form action="" method="post">
                        ...
                        <div class="col-md-8 score-filter-select ">
                            ...
                            <select id="assessment_form" name="assessment_form" class="form-control">
                                <option value="">*Select Assessment</option>
                                @foreach ($notify_id as $item)
                                    <option value="{{$item->id}}" name="{{$item->id}}">{{$item->a_name}}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-4 score-filter-button">
                            <input type="submit" class="btn btn-success getlistscore pull-right" value="Calculate Score"></button>
                        </div>
                    </form>
                </div>
            </div>
            @if (!empty($mdscore))
                ...
                <h1>MD Score: {{$mdscore}}</h1>
                </div>
                <div class="loader" style="display:none"><img src="/uploads/loader.gif" /></div>
            @endif
            @if (!empty($mdselfscore))
                <div class="dataresult">
                <h1>MD Self Score: {{$mdselfscore}}</h1>
                </div>
            @endif
        </div>
    </div>
